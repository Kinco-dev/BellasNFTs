...
contract Bellascoin is ERC20, Ownable {
    ...
    uint256 public maxSellLimit = 10_000_000 *10**18; // 1%
    uint256 public swapThreshold =  500_000* 10**18; // 0.05%

    // all known liquidity pools 
    mapping (address => bool) public automatedMarketMakerPairs;

    address payable public marketingWallet = payable(0x9859Ad9A4456B5e86478Ba37e0dc92a53a2Ee937); 
    address payable public cexWallet = payable(0xa684fA16858C5CD7D0bfB3208fA58C0eEad80AFd);
    address payable public devWallet = payable(0x79205F9C656761E29C045fEA2440096e2337a4Ba);
    ...
    constructor() ERC20("Bellascoin", "BELLAS") {
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BELLAS: Account has already the value of 'excluded'");
    ...
    function excludeFromMaxSellLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxSellLimit[account] != excluded, "BELLAS: Account has already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != dexPair, "BELLAS: The main pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BELLAS: Automated market maker pair is already set to that value");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(dexRouter), "BELLAS: The router has already that address");
    ...
    function setMaxSellLimit(uint256 amount) external onlyOwner {
        require(amount >= totalSupply()/1000/10**18, "BELLAS: Amount must be greater than 0.1% of the total supply");
    ...
    function setDevallet(address payable newWallet) external onlyOwner {
        require(newWallet != devWallet, "BELLAS: The dev wallet has already this address");
        emit DevWalletUpdated(newWallet,devWallet);
        devWallet = newWallet;
    }

    function setCexWallet(address payable newWallet) external onlyOwner {
        require(newWallet != cexWallet, "BELLAS: The cex wallet has already this address");
        emit CexWalletUpdated(newWallet,cexWallet);
        cexWallet = newWallet;
    }

    function setMarketingWallet(address payable newWallet) external onlyOwner {
        require(newWallet != marketingWallet, "BELLAS: The marketing wallet has already this address");
        emit MarketingWalletUpdated(newWallet,marketingWallet);
        marketingWallet = newWallet;
    }

    function setLiquidityWallet(address newWallet) external onlyOwner {
        require(newWallet != liquidityWallet, "BELLAS: The liquidity wallet has already this address");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "BELLAS: Transfer from the zero address");
        require(to != address(0), "BELLAS: Transfer to the zero address");
        require(amount >= 0, "BELLAS: Transfer amount must be greater or equals to zero");
        ...
        if(!_inSwapAndLiquify && isSellTransfer && from != address(dexRouter) && !_isExcludedFromMaxSellLimit[from])
            require(amount <= maxSellLimit, "BELLAS: Amount exceeds the maxSellTxLimit");
    ...
    function withdrawStuckMATIC(address payable to) external onlyOwner {
        require(address(this).balance > 0, "BELLAS: There are no Matic in the contract");
        to.sendValue(address(this).balance);
    } 

    function withdrawStuckERC20Tokens(address token, address to) external onlyOwner {
        require(token != address(this), "BELLAS: You are not allowed to get BELLAS tokens from the contract");
        require(IERC20(token).balanceOf(address(this)) > 0, "BELLAS: There are no tokens in the contract");
